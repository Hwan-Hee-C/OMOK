...
public class Session
{
    ...
    private void CreatePacket()
    {
        ...
        while (true)
        {
            ...
            OnRecv?.Invoke(packetHeader.Id, packetBytes.AsMemory(PacketHeader.HeaderSize));
        }
    }

    private async Task SendAsync(byte[] sendBuffer, CancellationToken cancellationToken)
    {
        NetworkStream stream = _client.GetStream();
    ...
    public Task SendPacketAsync(ushort packetId, ReadOnlySpan<byte> payload, CancellationToken ct)
    {
        ushort size = (ushort)(PacketHeader.HeaderSize + payload.Length);
        var header = new PacketHeader(size, packetId);

        byte[] buffer = new byte[size];
        PacketHeader.HeaderToBytes(header).CopyTo(buffer, 0);
        payload.CopyTo(buffer.AsSpan(PacketHeader.HeaderSize));

        return SendAsync(buffer, ct);
    }
